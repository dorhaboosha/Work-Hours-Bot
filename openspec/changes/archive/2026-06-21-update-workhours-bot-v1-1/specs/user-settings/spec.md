...
The system SHALL allow a user to create their work settings via the `/setup` command (`POST /settings/setup`). Settings consist of `dailyRequiredMinutes` (integer > 0), `timezone`, `workdays` (weekday numbers 0–6, where 0 = Sunday), and `language` (`en` or `he`). Settings are keyed by `telegramId`, and a user has at most one settings record. `/setup` is first-time setup only and SHALL NOT overwrite existing settings.

#### Scenario: Create new settings
- **WHEN** a user without existing settings submits valid setup input
- **THEN** a `user_settings` record is created for their `telegramId` including the selected `language`
...
#### Scenario: User chooses workdays, timezone, and language
- **WHEN** the bot guides the user through setup
- **THEN** it requires the user to choose or confirm workdays, timezone, and language
- **AND** it does not silently assume fixed workdays or timezone

### Requirement: Retrieve User Settings
The system SHALL allow retrieval of a user's work settings via `/settings` (`GET /settings/:telegramId`), returning `dailyRequiredMinutes`, `timezone`, `workdays`, and `language`.

#### Scenario: Settings found
- **WHEN** a request is made for a `telegramId` that has settings
- **THEN** the response returns the settings (including `language`) with `success: true`
...
#### Scenario: language must be supported
- **WHEN** `language` is provided but is not `en` or `he`
- **THEN** the response is `400` with `error.code = "VALIDATION_ERROR"`

## ADDED Requirements

### Requirement: Edit User Settings
The system SHALL allow a user to update existing settings via `/settings_edit` (`PATCH /settings/:telegramId`). Any subset of `dailyRequiredMinutes`, `timezone`, `workdays`, and `language` MAY be provided, and at least one field MUST be present. The user SHALL NOT need to re-run `/setup` to change settings.

#### Scenario: Update a single setting
- **WHEN** a user with existing settings submits a valid partial update (e.g. `language`)
...
**Reason**: V1.1 requires the user to choose or confirm workdays, timezone, and language during guided setup; the bot must not silently assume defaults.
**Migration**: Existing settings rows are unaffected. New setups collect every field explicitly through the `/setup` flow.
