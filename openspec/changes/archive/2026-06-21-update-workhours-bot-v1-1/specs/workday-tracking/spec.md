...
**Reason**: V1.1 removes `/end HH:mm`; previous unfinished workdays are now fixed through the `/edit dd-mm` flow.
**Migration**: Use `/edit dd-mm` with the `SET_END_HOUR` action to close an open record from a previous date. The `MANUAL_END_TIME_REQUIRED` error code is retired.
