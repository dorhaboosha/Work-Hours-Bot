...
This API supports the backend operations used by the Telegram bot for tracking work hours, editing records, managing absences, and viewing weekly/monthly summaries.
...
- `language` stores the bot language selected by the user.
...
- Each workday must be a number between `0` and `6`.
- `language`, if provided, must be one of:
  - `en`
  - `he`
...
## 3.7 GET `/workdays/edit/:telegramId/:date`
...
## 3.8 PATCH `/workdays/edit/:telegramId/:date`
...
## 3.9 GET `/summaries/week/:telegramId`
...
## 3.10 GET `/summaries/month/:telegramId`
