...
V1.1 should not support multiple bot languages.

The bot messages should be in English only.

However, the project should still keep bot labels and message texts in a single JSON file.
...
This keeps the code cleaner because messages are not hardcoded inside every handler, but it avoids the complexity of multi-language support.

V1.1 should not include:
- Hebrew language support
- Language selection in `/setup`
- Language editing in `/settings_edit`
- Language field in user settings
...
## 2.14 Time Input Validation Rules

The bot must strictly validate all user-entered time inputs.

This applies to:
- Setting only end hour for an open workday
- Setting start and end hours through `/edit dd-mm`

### 2.14.1 Valid Time Format

A valid time must use `HH:mm` 24-hour format.
...
### 2.14.2 Valid Time Range

When the user enters start and end hours, the end hour must be after the start hour.
...
### 2.14.3 Set End Hour Validation

When the user edits an open workday and chooses "Set end hour", the entered end hour must be after the existing start hour.

Example:

```txt
Existing start: 20:03
Entered end: 17:30
```

This is invalid because `17:30` is before `20:03` on the same work date.
...
### 2.14.4 Service-Level Validation Rule

Time validation must exist in the service layer, not only in the Telegram conversation layer.

Reason:
The service layer protects the business rules even if the input comes from:
- Telegram
- HTTP API
- tests
- future dashboard

The system must never save a `WORK` record when:
...
### 3.3 Bot Labels Rule
The bot uses English messages only.

The project should keep user-facing bot labels and messages in a single JSON file, for example:

```txt
botLabels.json
```

The labels file is used to avoid hardcoding message text across many files.

This is not multi-language support.
...
### 4.2 Slash Command Rule

V1.1 keeps Telegram slash commands only.

The user should use commands such as:
...
Reason:
Keeping only slash commands makes the bot simpler and avoids confusion between commands and normal conversation replies.
...
### 6.7 Simplicity / Code Organization Decision

V1.1 should stay simple and easy to understand.

The code should avoid unnecessary abstraction and avoid multi-language complexity.

Recommended simplification rules:
- Keep the bot English-only.
- Keep all user-facing messages in one JSON labels file.
- Keep slash commands only.
- Keep validation rules in shared helpers where possible.
- Keep important business validation inside services.
- Split large conversation files into smaller flow files if they become hard to understand.

Recommended flow files:

```txt
bot/flows/setupFlow.ts
bot/flows/settingsEditFlow.ts
bot/flows/editDayFlow.ts
bot/utils/timeInputParser.ts
bot/utils/botLabels.ts
```

The goal is not to over-engineer the project, but to make the code easier to read, test, and maintain.
...
- The bot responds to all V1.1 commands:
  ...
  - `/month`
  - `/help`
- `/setup` collects daily required hours, workdays, and timezone.
...
- `/settings_edit` allows changing daily hours, workdays, and timezone.
...
- The bot uses English messages only.
- The bot keeps message labels in a JSON file instead of hardcoding messages everywhere.
- The bot uses slash commands only and does not support commands without `/`.
- Invalid time values such as `25:00` or `45:00` are rejected.
- Invalid time ranges such as `17:00-09:00` are rejected.
- Setting only an end hour for an open workday is rejected if the end hour is before or equal to the existing start hour.
- Time validation is enforced in the service layer.
- The code is simplified by removing multi-language logic.
